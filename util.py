...
def obtain_classification_data(norm=False):
    ...
    petal_dims = np.concatenate(
        [petal_dims_setosa, petal_dims_virginica], axis=0)

    petal_labels = np.concatenate(
        [iris.target[iris.target == 0], iris.target[iris.target == 2]])

    petal_data = np.concatenate(
        [petal_dims, petal_labels.reshape(100, 1)], axis=1)
    petal_data[50:, 2] = 1
